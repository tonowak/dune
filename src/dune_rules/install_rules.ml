...
module Meta_and_dune_package : sig
  ...
  let make_dune_package sctx lib_entries (pkg : Package.t) =
    let pkg_name = Package.name pkg in
    let ctx = Super_context.context sctx in
    let pkg_root = Install.Context.lib_dir ~context:ctx.name ~package:pkg_name in
    let lib_root lib =
      let subdir =
...
let gen_package_install_file_rules sctx (package : Package.t) =
  ...
  let action =
    let install_file =
      Path.Build.relative
        pkg_build_dir
        (install_file ~package:package_name ~findlib_toolchain:ctx.findlib_toolchain)
    in
    let open Action_builder.O in
    let entries =
      let+ () = install_file_deps
